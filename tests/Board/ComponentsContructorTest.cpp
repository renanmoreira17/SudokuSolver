...
static DefaultComponentsConstructor instance;
...
TEST_CASE("Create Tile from ComponentsConstructor", "[ComponentsConstructor]")
{
    ...
    SECTION("Check if Grid address was passed to it") { REQUIRE(tile->getGrid() == mocGrid); }
...
TEST_CASE("Create Line from ComponentsConstructor", "[ComponentsConstructor]")
{
    ...
    REQUIRE(line);
    SECTION("Check if index was passed to it") { REQUIRE(line->getIndex() == 3); }
    SECTION("Check if Grid address was passed to it") { REQUIRE(line->getGrid() == &grid); }
...
TEST_CASE("Create Subgrid from ComponentsConstructor", "[ComponentsConstructor]")
{
    ...
    SECTION("Check if Grid address was passed to it") { REQUIRE(subgrid->getGrid() == &grid); }
    SECTION("Check if index was passed to it") { REQUIRE(subgrid->getIndex() == 4); }
}
